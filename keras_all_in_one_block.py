...
class AllInOneBlock(kr.layers.Layer):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        #try:
            #self.permute_function = {0 : (lambda u, w: tf.matmul(u, w)),
                                     #1 : (lambda u, w: tf.nn.conv1d(u, w, 1, 'same')),
                                     #2 : (lambda u, w: tf.nn.conv2d(u, w, 1, 'same')),
                                     #3 : (lambda u, w: tf.nn.conv3d(u, w, 1, 'same'))
                                    #}[self.Ddim]
        #except KeyError:
        ...
        if permute_soft:
            w = special_ortho_group.rvs(channels).astype(np.float32)
        else:
